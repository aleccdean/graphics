...
function render() {
  ...
  shaderProgram.setUniform1i("atomTexture", 0);
...
function loadTexture() {
  ...
  const max = (size -1) * 0.5;
  for(let r = 0; r < size; ++r) {
    for (let c = 0; c < size; ++c) {
      let i = (r * size +c) * 4;
      const dx = c - (size-1) * 0.5;
      const dy = r - (size-1) * 0.5;
      const distance = Math.sqrt(dx*dx + dy*dy) / ((size-1) * 0.5);
      ...
      pixels[i + 0] = (normal.x + 1) * 0.5 * 255;
      pixels[i + 1] = (normal.y + 1) * 0.5 * 255;
      pixels[i + 2] = (normal.z + 1) * 0.5 * 255;
      pixels[i + 3] = distance * 255;
    }
  }

   createRgbaTexture2d(size, size, pixels);
}
